...
\subsubsection{Filtering redundancy and weakest readings} \label{sec:redund-elim}

\paragraph{Note (updated for Utool 3.4).}
This section describes the filtering interface and rule syntax used by
Utool 3.4. Earlier versions of this manual documented the
\verb?--equivalences? (\verb?-e?) option and an XML equation format.
Utool 3.4 uses \verb?--filter? (\verb?-f?) and a term rewriting system
instead.
...
Utool filters a dominance chart without first enumerating its solved
forms. The filter computes relative normal forms with respect to two
kinds of rules: unoriented equations identify equivalent readings,
while oriented weakening rules select weaker readings. An annotator
supplies the polarity or other contextual information used to choose
weakening rules. Thus the current filter generalizes the older
equivalence-only redundancy elimination described in \cite{koller06}.
The annotated rewrite systems, relative normal forms, and their use
for computing weakest readings are described in detail by
\citeN{KollerThater2010Weakest}; the paper is available from the
\href{https://aclanthology.org/P10-1004/}{ACL Anthology}.

On the command line, pass the name of a rule file with
\verb?--filter <rulefile>? or \verb?-f <rulefile>?. Filtering applies
to both the \verb?solvable? and \verb?solve? commands. In particular,
the reported solution count is the size of the filtered chart, and
\verb?solve? enumerates the remaining solved forms.

\paragraph{Rule-file syntax.}
A rule file may contain annotator declarations, weakening rules, and
equivalence rules. Lines beginning with \verb?//? are comments. The
following is a complete small example:
...
The \verb?start annotation? is assigned at the root, and the
\verb?neutral annotation? is used where no polarity distinction is
required. A declaration such as \verb?+: a(+,+)? specifies the
annotations of the children of an \verb?a? node whose parent context
has annotation \verb?+?. A weakening rule has the form
\verb?[annotation] term -> term?; an equation has the form
\verb?term = term?. Variables start with an uppercase letter. An
explicit suffix such as \verb?#1? distinguishes multiple occurrences
of the same symbol in one rule. The notation \verb?*[term]? is a
wildcard context containing \verb?term?. Indices may be omitted when
each symbol occurs at most once and Utool can assign them
unambiguously.

\paragraph{Server mode.}
In server mode, put a \verb?filter? element inside the request and
pass the complete contents of the rule file in its \verb?rules?
attribute:

\begin{verbatim}
<utool cmd="solvable">
  <usr codec="domcon-oz" string="..."/>
  <filter rules="start annotation: +&#10;..."/>
</utool>
\end{verbatim}

Because the rules are an XML attribute value, the usual XML escaping
is required: in particular, write \verb?&amp;?, \verb?&lt;?, and
\verb?&quot;? for ampersands, less-than signs, and quotation marks.
Line breaks that separate rules or terminate comments must be written as
character references such as \verb?&#10;? because XML normalizes literal
line breaks in attributes to spaces.
An optional \verb?name? attribute supplies the label shown for the filtered
solution space in the desktop application.
After a request has supplied rules, a later request to the same server
process may use \verb?<filter/>? to reuse the most recently loaded
rule system. Using \verb?<filter/>? before any rules have been loaded
is an error.

\paragraph{Relationship to the historical ERG files.}
The repository contains two historical specifications that are useful
for reconstructing ERG filtering rules, but neither file is accepted
directly by the Utool 3.4 parser:

\begin{itemize}
\raggedright
\item The
\href{https://github.com/coli-saar/utool/blob/master/src/main/resources/examples/erg-equivalences.xml}{old ERG equivalence file}
is the XML input formerly used with \verb?-e?. It defines two groups
of mutually permutable label-hole pairs and two labels that permute
with everything.

\item The
\href{https://github.com/coli-saar/utool/blob/master/testsuites/rewriting-10/rules}{larger historical rules file}
combines equivalences with annotator and weakening rules. It uses an
intermediate shorthand in which \verb?/? introduces a child position,
\verb?=? states an equivalence, and \verb?>? states a weakening rule;
for example, \verb?label/1 = other/2? and
\verb?[+] label/1 > other/2?. Utool 3.4 instead requires the term
syntax shown above.
\end{itemize}

The larger file is not a complete conversion of the old ERG XML. Its
equivalence relation contains 12 of the XML file's 32 label-hole
entries and covers 66 of the 436 pairwise equivalences implied by the
two old groups. Twenty old entries, including \verb?every_q? and
\verb?each_q?, are absent. The two old ``permutes with everything''
entries for \verb?proper_q? and \verb?pronoun_q? are retained. The
files also use different hole numbering: the XML starts at hole 0,
while the shorthand rules start at position 1. A conversion intended
to reproduce the old ERG reduction must therefore translate the
syntax and restore or deliberately map the missing ERG predicates; a
mechanical syntax conversion of the larger rules file alone is not
sufficient.
...
\optiondesc{\texttt{-\strut -filter <rulefile>} or
\texttt{-f <rulefile>}}
{specify an element of the form \texttt{<filter rules="..."/>}
as a child of the \texttt{utool} element; after rules have been loaded,
\texttt{<filter/>} reuses them}
{\texttt{solvable}, \texttt{solve}}
{Filter redundancy and weakest readings from the chart (see
Section~\ref{sec:redund-elim}).}
