...
fn solve_falls_back_to_the_configured_default_output_codec() {
    ...
    assert!(
        String::from_utf8(output.stdout)
            .unwrap()
            .starts_with("%%  autogenerated by Utool\n[\n")
    );
}
...
#[test]
fn server_exits_with_a_clear_error_when_its_port_is_in_use() {
...
#[test]
fn server_uses_the_shared_user_configuration_port_by_default() {
    let listener = TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = listener.local_addr().unwrap().port();
    let directory = std::env::temp_dir().join(format!("utool-cli-config-{}", std::process::id()));
    fs::create_dir_all(&directory).unwrap();
    fs::write(
        directory.join(".utool"),
        format!("utool.server.port={port}\n"),
    )
    .unwrap();
    let mut command = Command::new(env!("CARGO_BIN_EXE_utool"));
    command.arg("server");
    #[cfg(unix)]
