...
fn solution_as_domcon(solution: &Solution) -> String {
    ...
    if let Some(root) = solution.root() {
        ...
        while let Some(tree) = stack.pop() {
            ...
            let id = builder.ensure_node(node.name.clone());
            builder
                .set_label(id, node.label.clone())
            ...
            for child in solution.arena().get_children(tree) {
                ...
                let child_id = builder.ensure_node(child_node.name.clone());
...
fn execute(opts: &Options, op: Operation, source: &str) -> Result<u8, (String, u8)> {
    ...
    if op == Operation::Solve {
        ...
        for solution in chart.solutions().take(opts.limit.unwrap_or(usize::MAX)) {
            count += 1;
            if let Some(codec) = solve_output_codec.as_deref() {
                rendered.push(match codec {
        ...
        if let Some(codec) = solve_output_codec {
            let text = if codec == "domcon-oz" {
                format!(
                    "%%  autogenerated by Utool\n[\n{}\n]\n",
                    rendered.join("\n")
                )
            } else {
                let separator = if codec == "term-prolog" { "," } else { " " };
                format!("[{}]", rendered.join(&format!("{separator}\n")))
            };
            write_result(opts, &text)?;
        }
        if opts.statistics {
