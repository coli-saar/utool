 ...
 */
public class GNDomGraphLayout extends FragmentLayoutAlgorithm {
	...
	private Map<String, List<GNFragmentTower>> fragmentToTowers;
	...
	public void initialise(DomGraph gr, NodeLabels labels, Canvas canv) {
		...
		fragmentToTowers = new HashMap<String, List<GNFragmentTower>>();
	...
	private Cost fragmentBoxDFS(String fragment, Set<String> visited, 
		...
		Cost cost = co;
		List<GNFragmentTower> myTowers = new ArrayList<GNFragmentTower>();
		...
		if(visited.contains(fragment)) {
			...
			if(myDominanceParents.isEmpty() && (! myTowers.isEmpty())) {
				GNFragmentTower toRemove = myTowers.remove(0);
			...
			for( GNFragmentTower tower : myTowers ) {
	...
	private void translateFragments(Collection<String> frags, 
		...
		for( String frag : frags ) {
			...
			if( fragmentToTowers.containsKey(frag)) {
				for( GNFragmentTower tower : fragmentToTowers.get(frag) ) {
	...
	private void computeTowers(String base, 
			Set<String> visited, List<GNFragmentTower> towers) {
		...
		for(Edge edge : getFragInEdges(base)) {
			...
			if(!visited.contains(parentFrag)) {
				GNFragmentTower thisTower = new GNFragmentTower();
	...
	private void towerDFS(String base, String frag,  
			GNFragmentTower tower, Set<String> visited) {
		int numInEdges = 0;
