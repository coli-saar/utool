...
public class Utool {
	public static void main(String[] args) {
		...
		if (triviallyUnsolvable) {
			if (options.hasOptionStatistics()) {
				System.err
						.println("The graph has trivially unsolvable dominance edges within fragments.");
			}
			...
			if ((options.getOperation() == Operation.solve)) {
				if (!(options.getOutputCodec() instanceof MultiOutputCodec)
					...
					System.err
							.println("This output codec doesn't support the printing of multiple solved forms!");
				...
				try {
					...
					System.err
							.println("An error occurred while trying to print the results.");
					// e.printStackTrace();
		...
		case solvable:
			if (options.hasOptionNochart()) {
				if (options.hasOptionStatistics()) {
					System.err
							.print("Checking graph for solvability (without chart) ... ");
				}

				try {
					long start_solver = System.currentTimeMillis();
					boolean solvable = OneSplitSource.isGraphSolvable(options
							.getGraph());
					...
					if (solvable) {
						if (options.hasOptionStatistics()) {
							...
							System.err
									.println("Time to determine solvability: "
											+ time_solver + " ms");
						}
						...
						if (options.hasOptionStatistics()) {
							...
							System.err
									.println("Time to determine unsolvability: "
											+ time_solver + " ms");
						}
		...
		case solve:
			...
			try {
				...
				if (solvable) {
					...
					if (options.hasOptionStatistics()) {
						System.err.println("it is solvable.");
						printChartStatistics(chart, time_solver, options
								.hasOptionDumpChart(), graph);
					}
					...
					if (options.getOperation() == Operation.solve) {
						try {
							if (!options.hasOptionNoOutput()) {
								outputcodec.print_header(options.getOutput());
								outputcodec.print_start_list(options
										.getOutput());
							}
							...
							long count = 0;
							SolvedFormIterator it = new SolvedFormIterator(
									chart, options.getGraph());
							while (it.hasNext()) {
								...
								if (!options.hasOptionNoOutput()) {
									if (count > 1) {
										outputcodec
												.print_list_separator(options
														.getOutput());
									}
									outputcodec.encode(options.getGraph()
											.makeSolvedForm(domedges), options
											.getLabels().makeSolvedForm(
													domedges), options
											.getOutput());
								}
							}
							long end_extraction = System.currentTimeMillis();
							long time_extraction = end_extraction
									- start_extraction;
							...
							if (options.hasOptionStatistics()) {
								System.err.println("Found " + count
										+ " solved forms.");
								System.err.println("Time spent on extraction: "
										+ time_extraction + " ms");
								long total_time = time_extraction + time_solver;
								System.err.print("Total runtime: " + total_time
										+ " ms (");
								if (total_time > 0) {
									System.err.print((int) Math.floor(count
											* 1000.0 / total_time));
									System.err.print(" sfs/sec");
								}

								if (count > 0) {
									System.err.print("; " + 1000 * total_time
											/ count + " microsecs/sf)");
								}
							...
							System.err
									.println("Output of the solved forms of this graph is not supported by this output codec.");
							System.err.println(e);
							System.exit(e.getExitcode()
									+ ExitCodes.MALFORMED_DOMGRAPH_BASE_OUTPUT);
						} catch (IOException e) {
							System.err
									.println("An error occurred while trying to print the results.");
							// e.printStackTrace();
		...
		case convert:
			if (!options.hasOptionNoOutput()) {
				try {
					...
					options.getOutputCodec().encode(options.getGraph(),
							options.getLabels(), options.getOutput());
					options.getOutputCodec().print_footer(options.getOutput());
				} catch (MalformedDomgraphException e) {
					System.err
							.println("This graph is not supported by the specified output codec.");
					System.err.println(e);
					System.exit(ExitCodes.MALFORMED_DOMGRAPH_BASE_OUTPUT
							+ e.getExitcode());
				} catch (IOException e) {
					System.err
							.println("An I/O error occurred while trying to print the results.");
					System.err.println(e);
					System.exit(ExitCodes.IO_ERROR);
				}
			}
		...
		case classify:
			...
			if (options.getGraph().isHypernormallyConnected()) {
				...
				if (options.hasOptionStatistics()) {
					System.err
							.println("The graph is not hypernormally connected.");
				}
			}
		...
		case display:
			if (options.getGraph() != null) {
				Ubench.getInstance().addJDomGraphTab(options.getInputName(),
						options.getGraph(), options.getLabels());
			} else {
				Ubench.getInstance();
			}
		...
		case server:
			try {
				...
				System.err
						.println("An I/O error occurred while running the server: "
								+ e);
	...
	private static void printChartStatistics(Chart chart, long time,
			boolean dumpChart, DomGraph compactGraph) {
		System.err.println("Splits in chart: " + chart.size());
		if (dumpChart) {
			// System.err.println(chart);
			System.err.println(ChartPresenter.chartOnlyRoots(chart,
					compactGraph));
		}
		...
		System.err.println("Number of solved forms: "
				+ chart.countSolvedForms());
		System.err.println("");
	}
	...
	private static void displayHelp(Operation op) {
		if ((op == null) || (op.longDescription == null)) {
			System.err
					.println("Usage: java -jar Utool.jar <subcommand> [options] [args]");
			System.err
					.println("Type `utool help <subcommand>' for help on a specific subcommand.");
			System.err
					.println("Type `utool --help-options' for a list of global options.");
			System.err
					.println("Type `utool --display-codecs' for a list of supported codecs.\n");

			System.err.println("Available subcommands:");
			for (Operation _op : Operation.values()) {
				if (_op.shortDescription != null) {
					System.err.println(String.format("    %1$-12s %2$s.", _op,
							_op.shortDescription));
				}
			}
			...
			System.err.println("For more information, see "
					+ GlobalDomgraphProperties.getHomepage());
		} else {
			System.err
					.println("utool " + op + ": " + op.shortDescription + ".");
	...
	private static void displayVersion() {
		System.err
				.println("Utool (The Swiss Army Knife of Underspecification), "
						+ "version " + GlobalDomgraphProperties.getVersion());
		System.err
				.println("Created by the CHORUS project, SFB 378, Saarland University");
		System.err.println();
	}
	...
	private static void displayHelpOptions() {
		...
		System.err
				.println("  --help-options                    Displays this information about global options.");
		;
		System.err
				.println("  --display-codecs, -d              Displays all input and output filters.");
		System.err
				.println("  --display-statistics, -s          Displays runtime and other statistics.");
		System.err
				.println("  --no-output, -n                   Do not display computed output.");
		System.err
				.println("  --equivalences, -e <filename>     Eliminate equivalent readings.");
		System.err
				.println("  --version                         Display version and copyright information.");
	}
