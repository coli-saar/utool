    		...
    		if( graph.getData(node).getType() == NodeType.UNLABELLED
    			&& graph.getAdjacentEdges(node, EdgeType.TREE).isEmpty() ) {
    				if( graph.indeg(node) > 0 ) {
    					throw new MalformedDomgraphException("The graph is not normal (" + node + " is a nontrivial empty fragment)", errorcode);
    				} else {
    					if( emptyTopFragment == null ) {
    						// we allow a single empty top fragment by removing it from the graph
    						emptyTopFragment = node;
    					} else {
    						throw new MalformedDomgraphException("The graph is not normal (" + emptyTopFragment + " and " + node + " are multiple top fragments)", errorcode);
    					}
    				}
    			}
    	}
