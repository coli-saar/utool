    ...
    public ChartViewer(Chart c, DomGraph g,
        ...
        if (Ubench.getInstance().reduceAutomatically) {
            if (!dg.isNormal()) {
                ...
                reduceChart(Ubench.getInstance().getRelativeNormalFormsComputer(),
            ...
            public String getToolTipText(MouseEvent e) {
                ...
                if (realColumnIndex == 0) {
                    ...
                    if (subgraph != null && (!subgraph.isEmpty())) {
                        int num = chart.countSolvedFormsFor(rootsToSubgraphs.get(subgraph)).intValue();
                        if (num == 1) {
...
//					updating cellview of the last selcted field
    ...
    void resetChart() {
        ...
        if (Ubench.getInstance().reduceAutomatically) {
            reduceChart(
                    Ubench.getInstance().getRelativeNormalFormsComputer(),
                    Ubench.getInstance().getEqsname());
        }
        refreshChartWindow();
        if (reduced) {
     ...
     */
    void reduceChart(RelativeNormalFormsComputer rnfc, String eqsn) {
...
//            }
    ...
    class ChartViewerMenu extends JMenuBar {
        ...
        ChartViewerMenu(ChartViewerListener li) {
            ...
            elredglobal = new JMenuItem("Filter with global rule system");
            elredglobal.addActionListener(lis);
            elredglobal.setActionCommand("elredglobal");
            elredglobal.setEnabled(Ubench.getInstance().isRelativeNormalFormsComputerLoaded());
