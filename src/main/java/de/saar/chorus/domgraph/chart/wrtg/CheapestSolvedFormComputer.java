...
public class CheapestSolvedFormComputer<E extends GraphBasedNonterminal, T extends Comparable<T>> {
	...
	private final WeightedRegularTreeGrammar<E, T> grammar;
	private final DomGraph graph;
	private T cost;
	private SolvedFormSpec form;
	private final Map<E,T> subgraphToCost;
	private final Map<Split<E>,T> splitToDerivationCost;
	private final Semiring<T> semiring;
	private final Map<E, String> root;
	private final Map<E, List<DomEdge> > nonterminalToDomEdges;
	private final List<DomEdge> domedges;
	private final PriorityQueue<SubgraphHeapMember<E,T>> queue;
	private final SubgraphHeapMember<E,T> sink;
	private final Map<E, SubgraphHeapMember<E,T>> ntToSHM;

	private final Map<Split<E>,Integer> splitToCounter;
	...
	private void computeCheapestForm() {
		...
		for(E nt: grammar.getAllNonterminals()) {
			...
			//if(grammar.isFinal(nt)) { -- CHECKME
			if(grammar.isSingleton(nt)) {
	...
	private class SubgraphHeapMember<E extends GraphBasedNonterminal, T extends Comparable<T>>
		...
		private final E nonterminal;
		private T key;
		private final Map<SubgraphHeapMember<E,T>, Split<E>> rules;
