...
class CommandLineParser {
    ...
    public AbstractOptions parse(String[] args)
        ...
        if (getopt.hasOption('f')) {
            try {
                ...
                parser.read(new FileReader(new File(getopt.getValue('f'))), weakening, equivalence, annotator);
                ...
                ret.setOptionComputeRnfs(true);
                ret.setRnfComputer(rnfc);
            } catch (Exception ex) {
                throw new AbstractOptionsParsingException("An error occurred while reading the filtering rules file!", ex, ExitCodes.EQUIVALENCE_READING_ERROR);
            }
        }
    ...
    private static ConvenientGetopt makeConvenientGetopt() {
        ...
        getopt.addOption('f', "filter", ConvenientGetopt.REQUIRED_ARGUMENT,
                "Filter by redundancy and weakest readings", null);
